...
class AssetFareSessionCreateTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        # Explicit caller-approved idempotent POST /v2/session create. The
        # caller-generated session capability token is REQUIRED input (this tool does
        # NOT generate it) and is sent only in the X-AssetFare-Session-Token header.
        # Retrying with the same token + idempotency_key recovers the same session.
        ...
        try:
            ...
            result = AssetFareClient().session_create(
            ...
            yield self.create_text_message(f"AssetFare session_create unavailable ({exc}).")
            return
